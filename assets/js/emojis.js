fetch('/assets/emojis.json')
  ...
  .then(emojis => {
    ...
    while (currentNode = textNodes.nextNode()) {
      ...
      if (emojiPattern.test(originalText) || originalText.includes('+1') || originalText.includes('-1')) {
        ...
        Object.keys(specialCases).forEach(caseKey => {
          ...
          if (casePattern.test(remainingText)) {
            let caseMatch;
            while ((caseMatch = casePattern.exec(remainingText)) !== null) {
              fragment.appendChild(document.createTextNode(remainingText.slice(0, caseMatch.index)));
              ...
              remainingText = remainingText.slice(caseMatch.index + caseKey.length);
